...
fn compareEntries(a: *const pt.ProcessEntry, b: *const pt.ProcessEntry, field: settings.SortField, descending: bool) i32 {
	const res: i32 = switch (field) {
		...
		.pid => cmp(a.pid, b.pid),
		.cpu => cmp(a.cpu_percent, b.cpu_percent),
		.memory => cmp(a.working_set, b.working_set),
		.threads => cmp(a.threads, b.threads),
		.handles => cmp(a.handles, b.handles),
		.starttime => cmp(a.start_time, b.start_time),
		.priority => cmp(a.base_priority, b.base_priority),
		.disk_io => cmp(a.disk_io_rate, b.disk_io_rate),
		.private_bytes => cmp(a.private_bytes, b.private_bytes),
		.page_faults => cmp(a.page_faults_per_sec, b.page_faults_per_sec),
		.user => win32.StrCmpIW(@ptrCast(&a.user), @ptrCast(&b.user)),
		.cmdline => win32.StrCmpIW(@ptrCast(&a.cmdline), @ptrCast(&b.cmdline)),
		.arch => cmp(a.arch_machine, b.arch_machine),
		.session => cmp(a.session_id, b.session_id),
		.peak_working_set => cmp(a.peak_working_set, b.peak_working_set),
		.virtual_mem => cmp(a.virtual_size, b.virtual_size),
		.gdi_objects => cmp(a.gdi_objects, b.gdi_objects),
		.user_objects => cmp(a.user_objects, b.user_objects),
		.integrity => cmp(a.integrity_level, b.integrity_level),
		.ppid => cmp(a.parent_pid, b.parent_pid),
		.private_ws => cmp(a.private_working_set, b.private_working_set),
		.paged_pool => cmp(a.paged_pool, b.paged_pool),
		.nonpaged_pool => cmp(a.non_paged_pool, b.non_paged_pool),
		.io_read => cmp(a.io_read_rate, b.io_read_rate),
		.io_write => cmp(a.io_write_rate, b.io_write_rate),
		.io_other => cmp(a.io_other_rate, b.io_other_rate),
		.description => win32.StrCmpIW(@ptrCast(&a.description), @ptrCast(&b.description)),
		.company => win32.StrCmpIW(@ptrCast(&a.company), @ptrCast(&b.company)),
		.dpi => @as(i32, @intFromEnum(a.dpi_awareness)) - @as(i32, @intFromEnum(b.dpi_awareness)),
		.service => win32.StrCmpIW(@ptrCast(&a.services), @ptrCast(&b.services)),
		.gpu => cmp(a.gpu_percent, b.gpu_percent),
		.gpu_memory => cmp(a.gpu_memory, b.gpu_memory),
		.cpu_time => cmp(a.cpu_time, b.cpu_time),
		.elevated => cmp(a.elevated, b.elevated),
		.path => win32.StrCmpIW(@ptrCast(&a.path), @ptrCast(&b.path)),
		.window_title => win32.StrCmpIW(@ptrCast(&a.window_title), @ptrCast(&b.window_title)),
		.file_version => win32.StrCmpIW(@ptrCast(&a.file_version), @ptrCast(&b.file_version)),
		.product_version => win32.StrCmpIW(@ptrCast(&a.product_version), @ptrCast(&b.product_version)),
		.session_name => win32.StrCmpIW(@ptrCast(&a.session_name), @ptrCast(&b.session_name)),
		.package_name => win32.StrCmpIW(@ptrCast(&a.package_name), @ptrCast(&b.package_name)),
		.peak_virtual_mem => cmp(a.peak_virtual_size, b.peak_virtual_size),
		.peak_private_bytes => cmp(a.peak_private_bytes, b.peak_private_bytes),
		.peak_paged_pool => cmp(a.peak_paged_pool, b.peak_paged_pool),
		.peak_nonpaged_pool => cmp(a.peak_non_paged_pool, b.peak_non_paged_pool),
		.peak_threads => cmp(a.peak_threads, b.peak_threads),
		.hard_faults => cmp(a.hard_faults_per_sec, b.hard_faults_per_sec),
		.cycles => cmp(a.cycles_per_sec, b.cycles_per_sec),
		.kernel_time => cmp(a.kernel_time, b.kernel_time),
		.user_time => cmp(a.user_time, b.user_time),
		.total_page_faults => cmp(a.total_page_faults, b.total_page_faults),
		.io_read_ops => cmp(a.io_read_ops, b.io_read_ops),
		.io_write_ops => cmp(a.io_write_ops, b.io_write_ops),
		.io_other_ops => cmp(a.io_other_ops, b.io_other_ops),
		.total_io => cmp(a.total_io_bytes, b.total_io_bytes),
		.elapsed => cmp(a.elapsed_time, b.elapsed_time),
		.shared_ws => cmp(a.shared_working_set, b.shared_working_set),
		.parent_name => win32.StrCmpIW(@ptrCast(&a.parent_name), @ptrCast(&b.parent_name)),
		.private_bytes_delta => cmp(a.private_bytes_delta, b.private_bytes_delta),
		.working_set_delta => cmp(a.working_set_delta, b.working_set_delta),
		.handle_delta => cmp(a.handle_delta, b.handle_delta),
		.thread_delta => cmp(a.thread_delta, b.thread_delta),
		.virtualization => cmp(a.virtualization, b.virtualization),
		.app_container => cmp(a.app_container, b.app_container),
		.domain => win32.StrCmpIW(@ptrCast(&a.domain), @ptrCast(&b.domain)),
		.user_sid => win32.StrCmpIW(@ptrCast(&a.user_sid), @ptrCast(&b.user_sid)),
		.efficiency => cmp(a.efficiency_mode, b.efficiency_mode),
		.io_priority => cmp(a.io_priority, b.io_priority),
...
fn getProcessDpiAwareness(h: win32.HANDLE) pt.TmDpiAwareness {
	...
	const fn_ptr = g_gpda_fn orelse return .unaware;
	var awareness: i32 = 0;
