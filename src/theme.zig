...
export fn theme_update() callconv(.c) void {
...
export fn theme_is_dark() callconv(.c) win32.BOOL {
	return g_dark;
}

export fn theme_apply_titlebar(hwnd: win32.HWND) callconv(.c) void {
	const dark = g_dark;
	_ = win32.DwmSetWindowAttribute(hwnd, DWMWA_USE_IMMERSIVE_DARK_MODE, &dark, @sizeOf(win32.BOOL));
}

export fn theme_apply_listview(hwnd_list: win32.HWND) callconv(.c) void {
	if (g_dark != 0) {
...
export fn theme_apply_button(hwnd: win32.HWND) callconv(.c) void {
	_ = win32.SetWindowTheme(hwnd, if (g_dark != 0) L("DarkMode_Explorer") else L("Explorer"), null);
}

export fn theme_apply_treeview(hwnd_tree: win32.HWND) callconv(.c) void {
	if (g_dark != 0) {
...
export fn theme_ctl_color(hdc: win32.HDC) callconv(.c) win32.HBRUSH {
...
export fn theme_bg_brush() callconv(.c) win32.HBRUSH {
	if (g_dark == 0) return null;
	if (g_dark_brush == null) g_dark_brush = win32.CreateSolidBrush(DARK_BG);
	return g_dark_brush;
}
